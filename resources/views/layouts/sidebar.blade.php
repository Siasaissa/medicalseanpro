            ...
            <div class="profile-info-widget">
                <a href="{{ route('patient.settings') }}" class="booking-doc-img">
                    ...
                    @if(Auth::user()->profile && Auth::user()->profile->dp)
                        <img src="{{ asset(Auth::user()->profile->dp) }}"
                             alt="Profile Photo"
                             class="img-fluid rounded-circle">
                    @else
                        <img src="{{ asset('images/profile-06.jpg') }}"
                             alt="Default Avatar"
                             class="img-fluid rounded-circle">
                    @endif
                </a>
                <div class="profile-det-info">
                    <h3><a href="{{ route('patient.settings') }}">{{ Auth::user()->name }}</a></h3>
                    <div class="patient-details">
                        <h5 class="mb-0">Patient ID : PT000{{ Auth::user()->id }}</h5>
                    </div>
                    <span> {{ Auth::user()->profile->sex ?? 'not set' }} <i class="fa-solid fa-circle"></i> 
                        @if($age)
                            {{-- Assuming $age comes as "25.598 years 7.144 months", remove decimals --}}
                            @php
        ...
        <div class="dashboard-widget">
            <nav class="dashboard-menu">
                <ul>
                    ...
                    <li class="{{ Route::is('patient.appointment') ? 'active' : '' }}">
                    ...
                    <li class="{{ Route::is('patient.favourites') ? 'active' : '' }}">
                    ...
                    <li class="{{ Route::is('patient.wallet') ? 'active' : '' }}">
                    ...
                    <li class="{{ Route::is('patient.vitals') ? 'active' : '' }}">
                        <a href="{{ route('patient.vitals') }}">
                            <i class="isax isax-note-1"></i>
                            <span>Vitals</span>
                        </a>
                    </li>

                    <li class="{{ Route::is('patient.settings') ? 'active' : '' }}">
